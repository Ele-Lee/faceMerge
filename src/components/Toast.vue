...
<template>
    <transition name="fade">
        <div v-if='concurrent || text || loading'>
            ...
            <section class='toast' v-if='concurrent'>
                <div class='toast__concurrent'>
                    <p>(〒︿〒)</p>
                    <p>{{'当前活动过于火爆'}}</p>
                    <p>{{'请耐心等候并重试'}}</p>
                </div>
...
<script>
    export default {
        ...
        beforeMount() {
            this.$bus.$on('alert', (text, size, duration = 1400) => {
                // 先清除上一个计时器
            ...
            this.$bus.$on('concurrent', (text, size, duration = 2400) => {
                // 先清除上一个计时器
                if (this.timeout) clearTimeout(this.timeout);
