...
export default class Preload {
    ...
    start() {
        // this.vue.$audio.mute(this.vue.$cache.load('muted', false))// 初始化是否静音
        this._initResource()
        Promise.all([
            // this._preloadAudio(),
            this._preloadImg(),
            this.vue.$api.getWxSignPackage(), // 平台项目用wxsdk.getWxSignPackage(),
            // this.$api.getWxUserInfo() // 平台项目用wxsdk.getWxUserInfo()
        ]).then(arr => {
            console.log(arr)
            ...
            // this.$bus.userInfo.name = arr[2].user_name
            // this.$bus.userInfo.avatar = arr[2].headimgurl
            // this.$bus.userInfo.openId = arr[2].openid
            // 加载完毕
    ...
    _initResource() {
        ...
        requireComponent.keys().forEach(fileName => {
            if(/result_list/.test(fileName)) return;
    ...
    _preloadAudio() {
        return new Promise((resolve, reject) => {
            this.vue.$audio.addLoadListener((loaded) => {
